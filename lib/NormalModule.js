...
class NormalModule extends Module {
    ...
    createLoaderContext(resolver, options, compilation, fs) {
        const loaderContext = {
            ...
            exec: (code, filename) => {
                ...
                module.paths = NativeModule._normalModulePaths(this.context);
    ...
    build(options, compilation, resolver, fs, callback) {
        this.buildTimestamp = new Data().getTime();
        this.built = true;
        this._source = null;
        this.error = null;
    ...
    sourceBlock(block, avaiableVars, dependencyTemplates, source, outputOptions, requestShortener) {
        ...
        const vars = block.variables.map((variable) => this.sourceVariables(
            variable, avaiableVars, dependencyTemplates, outputOptions, requestShortener))
            .filter(Boolean);

        /**
         * if we actually have variables
         * this is important as how #splitVariablesInUniqueNamedChunks works
         * it will always return an array in an array which would lead to IIFE wrapper around
    ...
    needRebuild(fileTimestamps, contextTimestamps) {
        ...
        // if the highest is Infinity, we need a rebuild
        // exit early here.
        ...
        // Again if the highest is Infinity, we need a rebuild
        // exit early here.
        if (highestFileDepTimestamp === Infinity) {
            return true;
        }
