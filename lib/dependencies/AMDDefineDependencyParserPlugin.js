    ...
    apply(parser) {
            ...
            switch(expr.argument.length) {
                ...
                case 2:
                    if (expr.arguments[0].type === "Literal") {
                        namedModule = expr.argument[0].value;
                        // define("...", ...)
        ...
        parser.plugin("call define:amd:item", (expr, param, namedModule) => {
            if (param.isConditional()) {
                ...
                } else if (localModule = LocalModuleHelpers.getLocalModule(parser.state, param.string, namedModule)) { // eslint-disable-line no-cond-assign
        ...
        parser.plugin("call define:amd:context", (expr, param) => {
            ...
            if (!dep) return;
            dep.loc = expr.loc;
            dep.options = !!parser.scope.inTry;
            parser.state.current.addDependency(dep);
            return true;
        });
    }
